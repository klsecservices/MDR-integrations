...
class MDRConsole():
    ...
    INCIDENTS_CLOSE_PATH = "incidents/close"
    INCIDENTS_COUNT_PATH = "incidents/count"
    INCIDENTS_CREATE_PATH = "incidents/create"
    INCIDENTS_DETAILS_PATH = "incidents/details"
    INCIDENTS_HISTORY_PATH = "incidents/history"
    ...
    def get_assets_count(self, **kwargs) -> Dict[str, Any]:
        ...
        result = self.post(path = self.ASSETS_COUNT_PATH, json_data = kwargs, headers=headers)
        return result
    ...
    def close_incident(self, incident_id: str, resolution_status: str, summary: str) -> Dict[str, Any]:
        ...
        kwargs = {
            ...
            "summary": "Some summary about the incident closing"
        }
        """
        headers = self.get_auth_header(self.access_token)
        kwargs = {}
        ...
        result = self.post(path = self.INCIDENTS_CLOSE_PATH, json_data = kwargs, headers=headers)
        return result
    

    def create_incident(self, affected_hosts: List[str], client_description: str, summary: str, **kwargs) -> Dict[str, Any]:
        """
        Example:
        kwargs = {
            "affected_hosts": "2NJMGXkBNGNeZ5iut24S",  # required
            "client_description": "something about the incident",  # required
            "markdown_to_html": True,
            "no_sla_flag": True,
            "priority": "HIGH",
            "summary": "Some summary about the incident",  # required
            "tenant_id": "1D12MGXkNKQ1Z5iut24S"
        }
        """
        headers = self.get_auth_header(self.access_token)
        kwargs['affected_hosts'] = affected_hosts
        kwargs['client_description'] = client_description
        kwargs['summary'] = summary
        result = self.post(path = self.INCIDENTS_CREATE_PATH, json_data = kwargs, headers=headers)
        return result
    ...
    def get_incidents_history(self, **kwargs) -> Dict[str, Any]:
        """
        Example:
        kwargs = {
            "entity_type_page_size": 100,
            "ignore_self": True,
            "incident_id": "2NJMGXkBNGNeZ5iut24S",
            "max_record_time": 1519640509000,
            "min_record_time": 1519640509000,
            "page": 1
        }
        """
        headers = self.get_auth_header(self.access_token)
        result = self.post(path = self.INCIDENTS_HISTORY_PATH, json_data = kwargs, headers=headers)
        return result
